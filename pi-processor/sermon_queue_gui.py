#!/usr/bin/env python3
"""
Sermon Processor for Raspberry Pi
Monitors MinIO bucket for new WAV files and converts them to AAC for streaming
"""
...
class DiscordNotifier:
    ...
    def send_conversion_complete(self, filename: str, duration: float, size_reduction: float):
        """Send notification when conversion is complete"""
        self.send_notification(
            title="<� Audio Conversion Complete",
            description=f"Successfully converted {filename}",
            color=0x00ff00,
            fields=[
                {"name": "Duration", "value": f"{duration:.1f} seconds", "inline": True},
                {"name": "Size Reduction", "value": f"{size_reduction:.1f}%", "inline": True},
            ]
        )
        
    def send_conversion_error(self, filename: str, error: str):
        """Send notification when conversion fails"""
        self.send_notification(
            title="L Conversion Error",
            description=f"Failed to convert {filename}",
            color=0xff0000,
            fields=[
    ...
    def send_processor_started(self, files_pending: int):
        """Send notification when processor starts"""
        self.send_notification(
            title="=� Sermon Processor Started",
            description="Pi processor is monitoring for new files",
            color=0x3498db,
            fields=[
                {"name": "Pending Files", "value": str(files_pending), "inline": True},
...
class SermonProcessor:
    """Main processor class for converting WAV to AAC"""
    ...
    def process_file(self, wav_object_name: str) -> bool:
        ...
        try:
            ...
            self.notifier.send_conversion_complete(
                filename,
                conversion_time,
                size_reduction
            )
            ...
            self.notifier.send_conversion_error(filename, str(e))
            return False
    ...
    def monitor_loop(self):
        ...
        pending = self.get_pending_files()
        self.notifier.send_processor_started(len(pending))
