...
echo "  • Automatic: Will prompt to watch after pushing to master"
echo "  • Manual check: ./check-deployment.sh"  
echo "  • Real-time: gh run watch (requires GitHub CLI)"
echo ""
echo "💰 Cost-Saving Features:"
echo "  • Pre-deployment check: ./pre-deploy-check.sh"
echo "  • Automatic Pi verification before push (saves GitHub Actions minutes)"
echo "  • Local issue detection prevents remote deployment failures"